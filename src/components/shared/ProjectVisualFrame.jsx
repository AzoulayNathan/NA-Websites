...
function AtelierNovaOverlay() {
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-90" viewBox="0 0 400 300" preserveAspectRatio="none">
      <rect x="220" y="40" width="160" height="120" fill="none" stroke="#6B5A4A" strokeWidth="0.8" opacity="0.22"/>
      <line x1="220" y1="100" x2="380" y2="100" stroke="#6B5A4A" strokeWidth="0.5" opacity="0.18"/>
      <line x1="220" y1="70" x2="380" y2="70" stroke="#6B5A4A" strokeWidth="0.5" opacity="0.14"/>
      <line x1="300" y1="40" x2="300" y2="160" stroke="#C4A574" strokeWidth="0.5" opacity="0.2"/>
    </svg>
  );
}
...
function getOverlay(project) {
  ...
  switch (project.slug) {
    ...
    case 'atelier-nova-habitat': return <AtelierNovaOverlay />;
...
function useImageFallback(slug, primarySrc) {
  const [idx, setIdx] = useState(0);
  const chain = primarySrc ? [primarySrc, ...assetErrorFallbacks(slug, primarySrc)] : [];
  useEffect(() => {
    setIdx(0);
  }, [slug, primarySrc]);
  ...
  return { src, onError, exhausted: !src && chain.length > 0 };
}
...
export default function ProjectVisualFrame({
  ...
  const imageFallback = useImageFallback(
    slug,
    asset?.type === 'image' ? asset.src : null,
  );
    ...
    <div className={`relative overflow-hidden ${className}`}>
      <motion.div
        ...
        {...(instant
          ...
          : { whileInView: { clipPath: 'inset(0 0% 0 0)' }, viewport: { once: true, margin: '-5%' } })}
        transition={{ duration: instant ? 0.85 : 1.1, delay: revealDelay, ease: [0.22, 1, 0.36, 1] }}
      ...
      >
        {asset?.type === 'video' ? (
          ...
          />
        ) : asset?.type === 'image' && imageFallback.src ? (
          <img
          ...
          />
        ) : asset?.type === 'image' && imageFallback.exhausted ? (
